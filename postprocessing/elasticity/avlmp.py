...
def prc_log(pathT, split):
    ...
    if split == 'n':
        subprocess.call(['python2.7 log2txt.py log.lammps data_out.txt > log.out'], shell=True, cwd=pathT)
    elif split == '3n':
        subdir = ['x', 'y', 'yz']
        for sub in subdir:
            pathsub = pathT + sub + '/'
...
def distav(avars, thermo):
    ...
    # Fit the data for each phase to t-student distribution and save the distribution mean    
    for ik in range(nl-1):    
        t0 = logL[ik] + 1
        tf = logL[ik+1]
        for jk in range(nd):
