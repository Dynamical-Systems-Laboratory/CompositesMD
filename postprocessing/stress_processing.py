...
def compute_spatial_stress(fname, time, nbins, idir, wpos=[], all_steps=False, av_per_atom=False):
	...
	# Stress components
	directions = ['xx', 'yy', 'zz', 'xy', 'xz', 'yz', 'ke', 'lj', 'bd', 'kspace', 'virial',
					'ke_yy', 'lj_yy', 'bd_yy', 'kspace_yy', 'virial_yy',
 					'ke_zz', 'lj_zz', 'bd_zz', 'kspace_zz', 'virial_zz']
	# ..and their position in d file (starts from 0)
	columns = list(range(8,29))
...
def compute_spatial_stress_and_energy(fname, time, nbins, idir, wpos=[], all_steps=False, av_per_atom=False):
	''' Compute spatial distribution of stress and energy'''
	...
	# Stress components
	directions = ['xx', 'yy', 'zz', 'xy', 'xz', 'yz', 'ke_xx', 'ke_yy', 'ke_zz', 
					'virial_xx', 'virial_yy', 'virial_zz', 'potential', 'kinetic']
	# ..and their position in d file (starts from 0)
	columns = list(range(8,21))
	...
	for step, atoms, box in zip(time_steps, atoms_all_t, box_all_t):
		...
		if av_per_atom:
			...
			for key, value in temp_stress.items():
				if (key == 'potential') or (key == 'kinetic'):
